...
class ProviderController {
  constructor() {
    ...
    this.providerConfigs = {
      ...
      "ollama-cloud": {
        name: "Ollama Cloud",
        baseUrl: config.providers.ollamaCloud.baseUrl,
        hasApiKey: !!config.providers.ollamaCloud.apiKey,
        models: [],
        features: ["chat", "completion", "cloud", "multimodal"],
        status: "unknown",
        enabled: true,
      },
      gemini: {
    ...
    // Load Ollama Cloud API key from DB if present
    this.loadOllamaCloudApiKey();
  }
  ...
  async loadOllamaCloudApiKey() {
    try {
      const settings = await ProviderSettings.findOne({ provider: "ollama-cloud" });
      if (settings && settings.apiKey) {
        this.providerConfigs["ollama-cloud"].hasApiKey = true;
        this.providerConfigs["ollama-cloud"].baseUrl =
          settings.baseUrl || this.providerConfigs["ollama-cloud"].baseUrl;
        this.providerConfigs["ollama-cloud"].enabled =
          settings.enabled !== undefined ? settings.enabled : true;
        this.providerConfigs["ollama-cloud"]._apiKey = settings.apiKey;
      }
    } catch (err) {
      console.warn("Could not load Ollama Cloud API key from DB:", err.message);
    }
  }
    ...
    try {
      ...
      for (const provider of Object.keys(providers)) {
        if (provider === "mistral") {
          ...
          let models = [];
          try {
            const result = await ollamaCloudService.listModels();
            models = result.map((m) => m.id || m.name);
          } catch (err) {
            models = providers[provider].models || [];
          }
          providers[provider].availableModels = models;
  ...
  async updateProvider(req, res) {
    try {
      ...
      if (apiKey !== undefined) {
        ...
        // Save API key to DB for Mistral and Ollama Cloud
        if (provider === "mistral" || provider === "ollama-cloud") {
  ...
  async testConnection(req, res) {
    try {
      ...
      if ((provider === "mistral" || provider === "ollama-cloud") && !testApiKey) {
        // Always fetch the latest API key from DB for Mistral and Ollama Cloud
        const ProviderSettings = require("../models/ProviderSettings");
  ...
  async testProviderConnection(providerName, config, apiKey = null) {
    const services = {
      ...
      "ollama-cloud": require("../services/ollamaCloudService"),
      openrouter: require("../services/openrouterService"),
      mistral: require("../services/mistralService"),
  ...
  getProviderRecommendations(useCase) {
    const recommendations = {
      general: [
        ...
        { provider: "ollama-cloud", model: "kimi-k2.5:latest", reason: "Cloud-hosted open models" },
      ],
      coding: [
        {
          provider: "openai",
        ...
        },
        {
          ...
          reason: "Agentic coding model in the cloud",
        },
        {
          provider: "ollama",
          model: "codellama",
